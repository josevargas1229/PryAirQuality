...
const Clima: React.FC<ClimaProps> = ({ children, backgroundColor, contentBackgroundColor, parallaxHeaderHeight, renderForeground }) => {
    ...
    const Card = ({ fecha, iko, condicion, min, max }: { fecha: string, iko: string, condicion: string, min: number, max: number }) => (
    ...
    const CardDias = ({ temperatura, ico, vel, hora }: { temperatura: number, ico: string, vel: number, hora: string }) => {
        const date = hora;
    ...
    const Datos = ({ dato, valor }: { dato: string, valor: string }) => (
